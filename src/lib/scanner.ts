...
function isPropertySpecificUrl(url?: string) {
  ...
  try {
    ...
    if (host.includes('landwatch.com') || host.includes('land.com') || host.includes('landsofamerica.com') || host.includes('landandfarm.com')) {
      return /\/pid\/\d+|\/property\/|\/properties\/|\/listing\/|\/land\//.test(path) && !/\/search\b|\/my-landwatch\b|\/saved-search/i.test(path);
    }
    return false;
  } catch {
    return false;
...
export async function runScan(override?: { query?: string; maxResults?: number; sendAlerts?: boolean; notePrefix?: string; expandThreads?: boolean }) {
  ...
  // Honor the query saved on the Settings page; fall back to the safe default.
  const gmailQuery = override?.query || settings.gmailSearchQuery?.trim() || SAFE_GMAIL_QUERY;
  ...
  let needsLocationCount = 0;
  let parsedCandidates = 0;
  type SaveResult = 'created' | 'createdNeedsLocation' | 'noAcreage' | 'belowMinAcres' | 'duplicate' | 'outsideRadius';
  ...
  async function saveOne(parsed: ParsedListing, emailId: string): Promise<SaveResult> {
    ...
    const orConds = [
      ...
      // Fallback for leads without a strong identifier, so rescans don't multiply them.
      !parsed.sourceListingId && !listingUrl && !parsed.address ? { title: parsed.title, acreage: parsed.acreage } : undefined,
    ...
    // Only a VERIFIED location outside the radius disqualifies a lead. Leads we
    // can't place stay on the dashboard as "Needs location" instead of vanishing.
    if (distance != null && distance > radiusMiles) return 'outsideRadius';
